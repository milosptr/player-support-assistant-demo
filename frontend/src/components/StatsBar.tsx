...
const OPEN_ICON = (
  <svg aria-hidden="true" className="h-4 w-4 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
...
const IN_PROGRESS_ICON = (
  <svg aria-hidden="true" className="h-4 w-4 text-violet-400" viewBox="0 0 20 20" fill="currentColor">
    <path fillRule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm.75-13a.75.75 0 0 0-1.5 0v5c0 .414.336.75.75.75h4a.75.75 0 0 0 0-1.5h-3.25V5Z" clipRule="evenodd" />
  </svg>
);

const RESOLVED_ICON = (
  <svg aria-hidden="true" className="h-4 w-4 text-emerald-400" viewBox="0 0 20 20" fill="currentColor">
...
function StatCard({ label, value, accent, icon, onClick, active }: StatCardProps) {
  ...
  if (onClick) {
    return (
      <button
        className={`card-hover text-left ${base} ${border} ${interactive}`}
        onClick={onClick}
      >
        {content}
      </button>
  ...
  return (
    <div className={`card-hover ${base} ${border}`}>
      {content}
    </div>
  );
}
...
export default function StatsBar({ stats, activeStatus, onStatusFilter }: StatsBarProps) {
  ...
  return (
    <div className="grid grid-cols-4 gap-4">
      <StatCard label="Total Tickets" value={stats.total} accent="bg-teal-500" icon={TOTAL_ICON} />
      <StatCard label="Open" value={stats.open} accent="bg-gray-500" active={activeStatus === 'open'} onClick={toggle('open')} icon={OPEN_ICON} />
      <StatCard label="In Progress" value={stats.in_progress} accent="bg-violet-500" active={activeStatus === 'in_progress'} onClick={toggle('in_progress')} icon={IN_PROGRESS_ICON} />
      <StatCard label="Resolved" value={stats.resolved} accent="bg-emerald-500" active={activeStatus === 'resolved'} onClick={toggle('resolved')} icon={RESOLVED_ICON} />
    </div>
  );
}
